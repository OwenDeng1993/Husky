...
enum Result { OK, ERROR, MORE_DATA };
...
class HTTP {
   public:
    ...
    bool request(const char* method, const char* endUrl, const char* data, boost::property_tree::ptree* root,
                 const char* content_type = _APPLICATION_JSON);

    /// Generic request that parses the result in Json::Object.
    unsigned int request(const char* method, const char* endUrl, const char* data, boost::property_tree::ptree* root,
                         Result& result, const char* content_type = _APPLICATION_JSON);
